...
(defn rprop [prop & [default]]
  (getprop (str "perf-test.rabbitmq" prop)
           default))
...
(def ^:dynamic *amqp-port*         (rprop->int "port" com.rabbitmq.client.AMQP$PROTOCOL/PORT))
(def ^:dynamic *amqp-host*         (rprop      "broker-hostname"        "localhost"))
(def ^:dynamic *amqp-exchange*     (rprop      "default.exchange.name"  "tpc.rabbitmq.default.exchange.name"))
(def ^:dynamic *amqp-queue*        (rprop      "default.queue.name"     "tpc.rabbitmq.default.queue.name"))
(def ^:dynamic *amqp-rpc-exchange* (rprop      "default.rpc.queue.name" "tpc.rabbitmq.default.rpc.queue.name"))
(def ^:dynamic *amqp-rpc-queue*    (rprop      "default.rpc.queue.name" "tpc.rabbitmq.default.rpc.queue.name"))
(def ^:dynamic *amqp-timeout*      (rprop->int  "timeout" 250))

(def ^:dynamic *amqp-message-properties*
...
(def ^:dynamic #^{:doc "Var for holding a default factory"}
     *amqp-factory*
     (ConnectionFactory. *connection-params*))

(def ^:dynamic #^{:doc "Var used to hold the connection to the factory."}
     *factory* nil)

(def ^:dynamic #^{:doc "Var used to hold the open connection on the broker."}
     *connection* nil)

(def ^:dynamic #^{:doc "Var used to hold the open channel on the connection."}
     *channel* nil)

;; TODO: document more of the options and parameters that have meaning
;; in the *env*
(def ^:dynamic #^{:doc "Var used to hold the current 'environment' for connections."}
