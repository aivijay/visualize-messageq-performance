(ns messageq-perf-test.perf-test
  (:import [java.util Date]
           [java.io File])
  (require [clojure.contrib.duck-streams :as ds])
  (require [messageq-perf-test.utils :as kutils])
  (:use [messageq-perf-test.rabbitmq :as rabbit]
        ;;[incanter2.utils    :as kutils]
        ;;[clojure.contrib.duck-streams    :only [reader]       :as ds]
...
(def ^:dynamic *working-directory* (kutils/$HOME "/tmp/amqp-benchmark"))
...
(def ^:dynamic *testing-info* {:broker "Apache QPID"
                     :series 1
                     :run-number 0
                     :num-producers  1
                     :num-consumers  1
...
(def ^:dynamic *producer-stats-file* (format "%s/producer-stats.tab" *working-directory*))

(defn ensure-producer-stats-file []
  (when (not (kutils/exists? *producer-stats-file*))
...
(def ^:dynamic *consumer-stats-file* (format "%s/consumer-stats.tab" *working-directory*))

(defn ensure-consumer-stats-file []
  (when (not (kutils/exists? *consumer-stats-file*))
    (kutils/with-out-appender
     *consumer-stats-file*
     (println (to-tab*
               "TEST-SERIES"
...
(def consumer-processor (future (Thread/sleep 100) (prn "Consumer awaking...") (consumer "c1") true))

;; TODO: need to have the consumer await the first message and not
;; just exit, then we wont' need the (Thread/sleep 100)
(defn ^:dynamic run-single-benchmark [series broker num-runs num-msgs num-producers]
...
(def test-msg-counts [1 5 10 50 100 500 1000 5000])
(defn ^:dynamic run-benchmark [broker num-prods num-cons]
  (doseq [msg-count test-msg-counts]
...
(defn simple-xy-plot [broker xname yname]
  (let [rabbit-data (get-xy-data broker xname yname)]
    (line-chart (rabbit-data 0) (rabbit-data 1)
...
'(let [mem-sizes (map #(Double/parseDouble (nth % 4))
      ...
      plot (line-chart counts mem-sizes )]
...
(defn run-perf-test [& args]
  (clear-stats)
  (run-benchmark "Apache-Qpid" 1 1))

(defn view-perf-test-plot []
    (view (simple-xy-plot "Apache-Qpid" "TOTAL-MESSAGES" "M/S")))
...
(defn -main [& args]
  (prn "Starting performance test")
  (performance-test)
  (prn "Completed performance test"))
